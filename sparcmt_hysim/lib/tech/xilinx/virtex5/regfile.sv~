...
(* syn_maxfan=16 *) module xcv5_regfile (input iu_clk_type gclk, input rst, input regfile_read_in_type rfi, output regfile_read_out_type rfo, input regfile_commit_type rfc);
	...
	bit [7:0]  rparity[0:1];	//time multiplexed read parity, parity[2:0] are packed in a separate ECC BRAM in case ECC is used to protect register file
	bit [7:0]  wparity;	//time multiplexed write parity 		
  ...
  assign op2_parity = (BRAMPROT > 0)? rparity[muxselectd] : '0;
    ...
    for (m=0; m<nmux; m++) begin
      for (i=0;i<nblock;i++) begin
          assign rdata[m][i*bw+(bw-1):i*bw] = w_rdata[m*nblock+i][(bw-1):0];
          assign rparity[m][i] = (BRAMPROT) ? w_rparity[m*nblock+i][0] : '0;		  //not used if not in ECC mode	
