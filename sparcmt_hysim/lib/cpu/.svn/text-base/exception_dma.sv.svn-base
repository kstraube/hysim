...
task automatic decode_microcode_mode(input xc_reg_type r, input bit trap, input bit store_nop, output bit [NUPCMSB:0] upc, output bit ucmode);
`ifndef SYNP94
	...
	if (trap == 1'b1) begin		//trap
		...
		unique case(op)
		  ...
		  LDST: unique case(op3)	//only support LDST now
			ST, STA, STC, STCSR : begin micropc = UPC_ST; uc = ~store_nop; end
			STB, STBA: begin micropc = UPC_STB; uc = ~store_nop;end
			STH, STHA: begin micropc = UPC_STH; uc = ~store_nop;end
			STDFQ, STDCQ, STD, STDA, STDC: begin micropc = UPC_STD; uc = ~store_nop; end
			SWAP, SWAPA: begin micropc = UPC_SWAP; uc =~store_nop; end 				
			LDSTUB, LDSTUBA: begin micropc = UPC_LDST; uc =~store_nop; end	
			STF: begin micropc = UPC_STF; uc = ~store_nop; end
			STFSR: begin micropc = UPC_STFSR; uc = ~store_nop; end
			STDF: begin micropc = UPC_STDF; uc = ~store_nop; end	
...
`endif
	...
	FMT3: begin 
	  if (ADVDEBUGDMA)
	     dma_data = da1;
  	  unique case(op3)
		  WRY : if (rd1 >= 8 && ASREN == 1) begin	     //handle WRASR, can't clobber g0 - g7
             		    spr1 = '1;
          		    wreg1 = '1;
		        end
  //		WRTBR : begin rd1 = REGADDR_TBR; wreg1 = '1; spr1 = '1;  end
	...
	LDST: begin
		unique case(op3)
		ST, STA, STB, STBA, STH, STHA, STD, STDA, STF, STDF, STFSR: begin 
	...
	if (spr1 == 1'b1 || r.ts.rdmask == 1'b1)		//in special register
		rda1 = {r.ts.tid, REGADDRPAD0, rd1[2:0]};
	else
		rda1 = regaddr(r.ts.tid, cwp, rd1);
	rdaddr1 = rda1;

	if (spr2 == 1'b1 || r.ts.rdmask == 1'b1)
		rda2 = {r.ts.tid, REGADDRPAD0, rd2[2:0]};
	else
  ...
  we1 = wreg1 & ((|r.ts.inst[29:25]) | ignrd | trap) & (r.ts.run | r.ts.dma_mode) & (~r.invalid | trap )& (~r.ts.annul | r.ts.dma_mode) & ~replay;  //CALL only writes to 1 register
  we2 = wreg2 & (|r.ts.inst[29:25] | trap) & (r.ts.run | r.ts.dma_mode) & (~r.invalid | trap) & (~r.ts.annul  | r.ts.dma_mode) & ~replay;
 
	rdata1 = da1; rdata2 = da2; 	
	debug_dma_data = dma_data;
endtask

task automatic fp_rd_gen(input xc_reg_type  r,
            input bit                     trap,
            input bit                     replay,
            output bit [NFPREGADDRMSB:0]  rdaddr,
            output bit [31:0]             rdata1, rdata2,
            output bit [1:0]              we1, we2);
...
`endif            
	...
	LDST: unique case(op3)
	 ...
	 FMT3: if (op3 == FPOP1) 
	         unique case(opf)
	         FADDD, FSUBD, FMULD, FSMULD, FITOD, FSTOD: begin
	           da1 = r.fp_ex_res[63:32];
	           da2 = r.fp_ex_res[31:0];
	           wreg1 = '1;
	           wreg2 = '1;
	           end
	         FADDS, FSUBS, FMULS, FMOVS, FNEGS, FABSS, FITOS, FSTOI, FDTOI, FDTOS: begin
	           if (rd[0]) begin
	             wreg2 = '1;
	             da2 = r.fp_ex_res[63:32];
	           end
	           else begin
	             wreg1 = '1;
	             da1 = r.fp_ex_res[63:32];
	...
	if (trap | ~r.ts.run | r.ts.annul | replay) begin
	   we1 = '0;
	   we2 = '0;
	end
	else begin
...
//rett:	RETT 
task automatic sp_write(input xc_reg_type r, output bit npc, rett, psr, wim, y, pc, fsr, fsr_clr_ftt);
`ifndef SYNP94
	bit [1:0] op  = r.ts.inst[31:30];
	bit [2:0] op2 = r.ts.inst[24:22];
	bit [5:0] op3 = r.ts.inst[24:19];
	
	bit wnpc = '0, ret_t = '0, wrwim = '0, wrpsr = '0, wry = '0, wpc = '0, ldfsr = '0, clr_ftt = '0;
`else
	bit [1:0] op;
	bit [2:0] op2;
	bit [5:0] op3;
	
	bit wnpc, ret_t, wrwim, wrpsr, wry, wpc, ldfsr, clr_ftt;
	
	op  = r.ts.inst[31:30];
	op2 = r.ts.inst[24:22];
	op3 = r.ts.inst[24:19];
	
	wnpc = '0; ret_t = '0; wrwim = '0; wrpsr = '0; wry = '0; wpc = '0; ldfsr = '0; clr_ftt = '0;
`endif

	unique case (op)
	LDST: begin
	        if (FPEN) begin
	         if (op3 == LDFSR) ldfsr = '1;
	         if (op3 == STFSR && r.ts.ucmode == 1'b1)
	              clr_ftt = '1;
	        end
	      end
	CALL: wnpc = '1;
	...
	npc = wnpc; rett = ret_t;  psr = wrpsr; wim = wrwim; y = wry; pc = wpc; fsr = ldfsr; fsr_clr_ftt = clr_ftt;
...
module exception_stage_dma(input iu_clk_type gclk, input bit rst, 
		       ...
		       output	commit_reg_type 	   comr /* synthesis syn_preserve = 1 */,
		       output	bit			              luterr,
		       output	bit			              bramerr,
		       output	bit			              sb_ecc,
		       output bit                 irqack,
  ...
  //debug dma related signals
	...
	bit [31:0] dma_data;
        ...
        xc_reg_type xcr_mmu;      //patched xcr after mmureg load
 
	//pipeline register
	//commit_reg_type	cr, crin;
  commit_reg_type crin;
  (* syn_srlstyle="registers" *) commit_reg_type comr_r;

	bit		wnpc, rett, wrwim, wrpsr, wy, ldfsr, fsr_clr_ftt;	       //special write flags;
  ...
  (* syn_preserve=1 *) tm_cpu_ctrl_token_type    r_cpu2tm;
  ...
  always_comb begin
		sp_write(xcr, wnpc, rett, wrpsr, wrwim, wy, wpc, ldfsr, fsr_clr_ftt);

		//default values
		error_mode = '0;
		replay     = xcr.ts.replay | dcache2iu.replay | (MMUEN & dmmu2iu.replay);

		crin.spr.replay   = replay;	              //replay bit
		crin.spr.icmiss   = xcr.ts.icmiss;        //icache miss
		crin.spr.run      = xcr.ts.run;
		
		crin.spr.flushidx = xcr.ex_res[ICACHEINDEXMSB_MEM:ICACHEINDEXLSB_MEM];
		
		crin.spr.psr = xcr.ts.psr;
		crin.spr.fsr = xcr.ts.fsr;
		crin.spr.y   = xcr.ts.y;
		...
		trap_detected = (xcr.trap_res.precise_trap | dmmu2iu.exception | xcr.trap_res.irq_trap) & ~replay & ~xcr.ts.dma_mode & xcr.ts.run;    //assume microcode is well written?
    ...
    irqack = xcr.trap_res.irq_trap & ~replay & ~xcr.ts.dma_mode & xcr.ts.run & (tbr_tt[7:4] ==  4'b0001);

		//decode microcode mode	
		decode_microcode_mode(xcr, trap_detected, dmmu2iu.store_nop, crin.spr.upc, ucmode);
		
		//modify ucmode and upc, if no replay
		if (!replay) begin
			//crin.spr.ucmode = (xcr.ts.ucmode == 1'b1)? !xcr.ts.uend : !xcr.invalid & !xcr.ts.annul & ucmode;
			crin.spr.ucmode = (xcr.ts.ucmode == 1'b1)? ~xcr.ts.uend : (~xcr.ts.annul | xcr.ts.dma_mode) & ucmode;
			if (xcr.ts.ucmode)
    ...
    if (xcr.ts.icmiss | xcr.ts.dma_mode | xcr.ts.run == 1'b0)
      ...
      crin.spr.annul = ~xcr.invalid & ~xcr.ts.annul & xcr.annul_next & ~trap_detected;		
		...
		if (FPEN) begin
		  if (ldfsr == 1'b1 && !trap_detected) begin
		    crin.spr.fsr.rd    = xcr.mem_res[31:30];
		    crin.spr.fsr.tem   = xcr.mem_res[27:23];
		    crin.spr.fsr.ns    = xcr.mem_res[22];
		    crin.spr.fsr.fcc   = xcr.mem_res[11:10];
		    crin.spr.fsr.aexc  = xcr.mem_res[9:5];
		    crin.spr.fsr.cexc  = xcr.mem_res[4:0];
		  end
		  if (fsr_clr_ftt == 1'b1 && !trap_detected) // clear ftt after a STFSR instruction
		   crin.spr.fsr.ftt = '0;
		end
		...
		if (FPEN)
		  // added by rimas to support clearing of fsr.ftt after STFSR (which will happen in microcode mode)
		  crin.spr.archr_we |= (fsr_clr_ftt & xcr.ts.run & ~xcr.ts.annul & ~replay & ~xcr.ts.dma_mode); 
		  
		crin.spr.psr_we = crin.spr.archr_we;
		crin.spr.archr_we = crin.spr.archr_we & ~(trap_detected && xcr.trap_res.tbr_tt[7:4] == 1); // don't update archr on irq
		...
		xcr_mmu = xcr;
		if (MMUEN & dmmu2iu.mmureg_valid) xcr_mmu.mem_res[31:0] =  dmmu2iu.mmureg_read;
		
		rd_gen(xcr_mmu, tbr_tt, (MMUEN) ? xcr.ex_res[31:0] :  dmmu2iu.paddr, trap_detected, replay, error_mode, crin.spr.psr.cwp, crin.regf.ph1_addr, crin.regf.ph2_addr, crin.regf.ph1_data, crin.regf.ph2_data, crin.regf.ph1_we, crin.regf.ph2_we, dma_data);
		...
		if (FPEN) begin
		    fp_rd_gen(xcr, trap_detected, replay, crin.fpregf.ph_addr, crin.fpregf.ph1_data, crin.fpregf.ph2_data, crin.fpregf.ph1_we, crin.fpregf.ph2_we);
		    crin.fpregf.ph1_parity = '0; crin.fpregf.ph2_parity = '0;
		    if (BRAMPROT == 1) begin	//parity bit encoding
          crin.fpregf.ph1_parity[6] = ^crin.fpregf.ph1_data;
          crin.fpregf.ph2_parity[6] = ^crin.fpregf.ph2_data;
        end
    end
		
		//generate pc, npc;						
		//if (wnpc == 1'b1 && ((xcr.ts.annul_trap == 1'b0 && xcr.ts.ucmode == 1'b0) || xcr.ts.ucmode))	//wnpc = 1 => invalid = 0
		if (wnpc == 1'b1 && (((xcr.ts.annul == 1'b0 || xcr.ts.dma_mode == 1'b1) && xcr.ts.ucmode == 1'b0) || xcr.ts.ucmode))	//wnpc = 1 => invalid = 0
    ...
    iu2dma.state.data = dma_data;       //update the data field for LD, FMT3 result
	  ...
	  v_cpu2tm.run     = xcr.ts.valid & xcr.ts.run;
	  v_cpu2tm.retired = xcr.ts.valid & xcr.ts.run & cpu_stoppable & ~xcr.ts.dma_mode;
	  v_cpu2tm.replay  = xcr.ts.valid & xcr.ts.run & ~v_cpu2tm.retired;
	  v_cpu2tm.valid   = xcr.ts.valid & ~v_cpu2tm.replay;
	  v_cpu2tm.inst    = xcr.ts.inst;
	  v_cpu2tm.paddr   = dmmu2iu.paddr;
	  v_cpu2tm.npc     = {xcr.ts.npc,2'b0};
  end
...
`endif    
    ...
    if (rst == 1'b1) begin
      ...
      cr.spr.psr_we   = (SPRBRAM == 1'b0)? '1 : '0;		//WE for psr else
      ...
      if (ASR15EN) begin
        ...
        cr.regf.ph1_addr[NREGADDRMSB:NREGADDRMSB-NTHREADIDMSB] = xcr.ts.tid;
        cr.regf.ph1_addr[NREGADDRMSB-NTHREADIDMSB-1:0] = 15;
        cr.regf.ph1_data = unsigned'(xcr.ts.tid);
        cr.regf.ph1_parity[6] = ^xcr.ts.tid;
      end
      else
        cr.regf.ph1_we = '0;      
    end	
      ...
      cr.spr.pc_we    = (SPRBRAM == 1'b0)? '1 : '0;		//WE for pc
      cr.spr.npc_we   = (SPRBRAM == 1'b0)? '1 : '0;		//WE for npc
      cr.spr.archr_we = (SPRBRAM == 1'b0)? '1 : '0;		//WE for architecture registers else
      cr.spr.psr_we   = (SPRBRAM == 1'b0)? '1 : '0;		//WE for psr else      
      //preset value at reset
      cr.spr.pc  = '0;	cr.spr.npc = 1;			
