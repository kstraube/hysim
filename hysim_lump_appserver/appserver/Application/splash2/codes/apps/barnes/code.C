...
//   }
   ...
   if (NPROC > 1) {
       printf("about to request %d harts\n",NPROC-1);
       hart_request(NPROC-1);
   }
...
//   printf("about to call find_my_initial_bodies ProcessId = %d\n",ProcessId);
   find_my_initial_bodies(bodytab, nbody, ProcessId);
//   printf("done with find_my_initial_bodies\n");

   /* main loop */
   while (Local[ProcessId].tnow < tstop + 0.1 * dtime) {
//      printf("about to call stepsystem\n");
...
// moved this here since we need to know NPROC to initialize barriers
   BARINIT(Global->Barload);
   BARINIT(Global->Bartree);
   BARINIT(Global->Barcom);
   BARINIT(Global->Baraccel);
...
//  printf("about to wait on Global->Barstart NPROC=%d\n",NPROC);
  BARRIER(Global->Barstart,NPROC);
//  printf("left on Global->Barstart\n");
}
