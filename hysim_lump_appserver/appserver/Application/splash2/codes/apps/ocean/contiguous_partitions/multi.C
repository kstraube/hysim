...
/* max work exceeded                                               */
         flag1 = 1;
	 fmyprintf(stderr,"ERROR: Maximum work limit %0.5f exceeded\n",wmax);
	 exit(-1);
       } else {
...
/* remains what it already is                                      */
   ...
   if (do_output) {
     if (my_num == MASTER) {
       myprintf("iter %d, level %d, residual norm %12.8e, work = %7.3f\n",
   ...
   if (color == RED_ITER) {
     for (i=evenistart;i<iend;i+=2) {
       ...
       for (j=evenjstart;j<jend;j+=2) {
         ...
         #ifdef RECIPROCAL_APPROXIMATION
           newval = a * recip_factor;
         #else
           newval = a / factor;
         #endif
