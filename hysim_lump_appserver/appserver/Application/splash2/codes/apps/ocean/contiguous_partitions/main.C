...

{
   ...
   while ((ch = getopt(argc, argv, "n:p:e:r:t:soh")) != -1) {
     ...
     case 'h': myprintf("Usage: OCEAN <options>\n\n");
               myprintf("options:\n");
               myprintf("  -nN : Simulate NxN ocean.  N must be (power of 2)+2.\n");
               myprintf("  -pP : P = number of processors.  P must be power of 2.\n");
               myprintf("  -eE : E = error tolerance for iterative relaxation.\n");
               myprintf("  -rR : R = distance between grid points in meters.\n");
               myprintf("  -tT : T = timestep in seconds.\n");
               myprintf("  -s  : Print timing statistics.\n");
               myprintf("  -o  : Print out relaxation residual values.\n");
               myprintf("  -h  : Print out command line options.\n\n");
               myprintf("Default: OCEAN -n%1d -p%1d -e%1g -r%1g -t%1g\n",
   ...
   jm = im;
   myprintf("\n");
   myprintf("Ocean simulation with W-cycle multigrid solver\n");
   myprintf("    Processors                         : %1d\n",nprocs);
   myprintf("    Grid size                          : %1d x %1d\n",im,jm);
   myprintf("    Grid resolution (meters)           : %0.2f\n",res);
   myprintf("    Time between relaxations (seconds) : %0.0f\n",dtau);
   myprintf("    Error tolerance                    : %0.7g\n",tolerance);
   myprintf("\n");
...
//   }
   ...
   if (nprocs > 1)
       hart_request(nprocs-1);

   if (do_output) {
     myprintf("                       MULTIGRID OUTPUTS\n");
   }

   slave();
//   WAIT_FOR_END(nprocs-1)
   BARRIER(bars->done_barrier)
   CLOCK(computeend)


   myprintf("\n");
   myprintf("                       PROCESS STATISTICS\n");
   myprintf("                  Total          Multigrid         Multigrid\n");
   myprintf(" Proc             Time             Time            Fraction\n");
   myprintf("    0   %15.0f    %15.0f        %10.3f\n",
   ...
   if (do_stats) {
     ...
     for (i=1;i<nprocs;i++) {
       myprintf("  %3d   %15.0f    %15.0f        %10.3f\n",
	      i,gp[i].total_time,gp[i].multi_time,
	      gp[i].multi_time/gp[i].total_time);
     }
     myprintf("  Avg   %15.0f    %15.0f        %10.3f\n",
            avg_total,avg_multi,avg_frac);
     myprintf("  Min   %15.0f    %15.0f        %10.3f\n",
            min_total,min_multi,min_frac);
     myprintf("  Max   %15.0f    %15.0f        %10.3f\n",
            max_total,max_multi,max_frac);
   }
   myprintf("\n");

   global->starttime = start;
   myprintf("                       TIMING INFORMATION\n");
   myprintf("Start time                        : %16d\n",
           global->starttime);
   myprintf("Initialization finish time        : %16d\n",
           global->trackstart);
   myprintf("Overall finish time               : %16d\n",
           computeend);
   myprintf("Total time with initialization    : %16d\n",
           computeend-global->starttime);
   myprintf("Total time without initialization : %16d\n",
           computeend-global->trackstart);
   myprintf("    (excludes first timestep)\n");
   myprintf("\n");
