...
/*   for (i = 1; i < Number_Of_Processors; i++) {
   ...
   if (Number_Of_Processors > 1) { 
      ...
      hart_request(Number_Of_Processors-1);
   }

//   printf("About to call ParallelExecute()\n");

   ParallelExecute();
//   WAIT_FOR_END(Number_Of_Processors - 1);

   BARRIER(G_Memory->done_barrier);
   printf("Finished FMM\n");
   PrintTimes();
   if (do_output) {
     PrintAllParticles();
...
//   printf("starting ParallelExecute()\n");
    
   local_time = (time_info *) malloc(sizeof(time_info) * MAX_TIME_STEPS);
...
/* POSSIBLE ENHANCEMENT:  Here is where one might reset the
   ...
   BARRIER(G_Memory->synch, Number_Of_Processors);
//   printf("Exiting ParallelExecute()\n");
}
