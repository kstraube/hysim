...
void radiosity()
{
    ...
    printf("radiosity() - process_id = %d\n",process_id);

    debug("about to call init_modeling_tasks()\n");
    init_modeling_tasks(process_id) ;
    debug("about to call process_tasks()\n");
    process_tasks(process_id) ;
    
    /* Gather rays & do BF refinement */
    debug("about to call init_ray_tasks()\n");
    ...
    debug("about to call init_radavg_tasks()\n");
...
// added by rimas
    printf("initializing done_barrier: n_processors = %d\n",n_processors);
